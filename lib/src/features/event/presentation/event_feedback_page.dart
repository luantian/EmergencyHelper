...
class EventFeedbackPage extends StatefulWidget {
  const EventFeedbackPage({required this.eventId, super.key});
...
class _EventFeedbackPageState extends State<EventFeedbackPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: const Key('event-feedback-root'),
      appBar: AppBar(
        title: const Text('事件反馈'),
  ...
  Widget _buildIntroCard() {
    return Container(
      ...
      child: const Row(
        children: <Widget>[
          Icon(Icons.campaign_rounded, size: 18, color: Color(0xFF2D78C6)),
          SizedBox(width: 8),
          Expanded(
            child: Text(
              '请补充最新处置进展，支持上传图片或视频作为佐证材料。',
              style: TextStyle(
  ...
  Future<void> _onSubmit() async {
    ...
    try {
      ...
      await EventCenter.instance.submitFeedback(
      ...
      _showMessage(_friendlySubmitError(error.message));
    } catch (_) {
      await _cleanupUploadedAttachmentIds(uploadedAttachmentIds);
      _showMessage('提交反馈失败，请稍后重试');
    } finally {
      if (mounted) {
        setState(() {
          _submitting = false;
  ...
  String _friendlySubmitError(String message) {
    final text = message.trim();
    if (text.isEmpty) {
      return '提交反馈失败，请稍后重试';
    }
    if (text.contains('系统异常')) {
      return _attachments.isEmpty
          ? '反馈接口返回系统异常，请联系后台排查'
          : '反馈接口返回系统异常，请联系后台排查（附件已自动回收）';
    }
    return text;
  }
