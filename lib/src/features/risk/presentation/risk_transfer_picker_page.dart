...
class RiskTransferPickerPage extends StatefulWidget {
  const RiskTransferPickerPage({required this.riskId, super.key});

  final String riskId;

  @override
  State<RiskTransferPickerPage> createState() => _RiskTransferPickerPageState();
}

class _RiskTransferPickerPageState extends State<RiskTransferPickerPage> {
  ...
  Future<void> _initializePage() async {
    ...
    final permission = await AppFeaturePermissionResolver.instance.resolve(
      dependencies.authService,
      forceRefresh: true,
    );
    if (!permission.canRiskTransfer) {
      if (mounted) {
        AppCenterToast.show(
          context,
          '\u5F53\u524D\u8D26\u53F7\u65E0\u98CE\u9669\u8F6C\u6D3E\u6743\u9650',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: const Key('risk-transfer-picker-root'),
      appBar: AppBar(
    ...
    return PopupMenuButton<_TransferTreeMenuAction>(
      ...
      itemBuilder: (context) => const <PopupMenuEntry<_TransferTreeMenuAction>>[
        PopupMenuItem<_TransferTreeMenuAction>(
          ...
          child: Row(
            children: <Widget>[
              Icon(Icons.unfold_more_rounded, size: 18, color: Color(0xFF42566D)),
              SizedBox(width: 10),
              Text('全部展开'),
            ],
          ),
        ...
        PopupMenuItem<_TransferTreeMenuAction>(
          ...
          child: Row(
            children: <Widget>[
              Icon(Icons.unfold_less_rounded, size: 18, color: Color(0xFF42566D)),
              SizedBox(width: 10),
              Text('全部收起'),
            ],
          ),
  ...
  void _onConfirm() {
    ...
    final selectedNames = _allContacts
        ...
        .toList();
    Navigator.of(context).pop<List<String>>(selectedNames);
  }
}
